...
/* Initial state to hold from input, saving state*/
const initialState = {
    name: '',
    description: '',
    image: {},
...
export default function CreatePost({
    ...
    function onChangeFile(e) {
        ...
        updateFormState(currentState => (
            { ...currentState, file: URL.createObjectURL(e.target.files[0], image) }
        ))
    }
    ...
    async function save() {
        try {
            ...
            const postId = uuid();
            const postInfo = {
                name, description, location, image: formState.image.name, id: postId
            };
            /* put the image to S3 */
            await Storage.put(formState.image.name, formState.image.fileInfo);
            /* Store data with the api and dynamoDB */
            await API.graphql({
                query: createPost,
